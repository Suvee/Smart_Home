...
void gtp_sleep(int sleep)
{
	...
	gpio_direction_output(EXYNOS4_GPX0(3), sleep);
	gpio_free(EXYNOS4_GPX0(3));
#endif 
	void *reg;
	int reg_value;

	reg = ioremap(0x11000c00,8);
	reg_value = readl(reg);
	reg_value &= ~(0xf << 12);
	reg_value |=  (0x1 << 12);
	writel(reg_value,reg);

	reg_value = readl(reg + 4);
	reg_value &= ~(1 << 3);
	reg_value |= (sleep << 3);
	writel(reg_value,reg + 4);
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
    ...
    input_mt_init_slots(ts->input_dev, 255,0);//CYG MODIFY
#else
...
struct  of_device_id  gt818_match_table[] = {
	{	
		.compatible= "ts-gt818"
	},{}
...
static struct i2c_driver goodix_ts_driver = {
    ...
    .driver = {
        .name     = GTP_I2C_NAME,
        .owner    = THIS_MODULE,
		.of_match_table  = gt818_match_table,
    },
...
static int ctp_get_system_config(void)
{
    ...
    if(SCIRPT_ITEM_VALUE_TYPE_STR != script_get_item("ctp_para", "gt818_update_file_path", &item))
	{
		pr_err("%s: gt818_update_file_path script_get_item  err. \n", __func__);
		gt818_update_file_path = "/system/etc/";
	} 
	else
	{
		gt818_update_file_path = item.str;
		pr_info("%s:gt818_update_file_path:%s\n",__func__,gt818_update_file_path);
	}
